...
* @fnAfterGroupClicked                                  Function            Function that is called when grouping click is finished.
*/
(function ($) {
    ...
    $.fn.rowGrouping = function (options) {
        ...
        function _fnAfterGroupClicked(sGroup, iGroupLevel, sState) {
            ///<summary>
        ...
        var defaults = {
            ...
            fnAfterGroupClicked: _fnAfterGroupClicked,
        ...
        return this.each(function (index, elem) {
            ...
            function _fnCreateGroup2Row(sGroup2, sGroupLabel, iColspan, oParentGroup) {
                ...
                if (properties.bExpandableGrouping && properties.bExpandableGrouping2) {
                    ...
                    //Martin Ng - fix two level issue    
                    if (oParentGroup.state == "collapsed") {
                        $(nGroup2).hide();
                    }
            ...
            function _fnIsGroupCollapsed(sGroup) {
                if (aoGroups[sGroup] != null)
                    ...
                    if (sGroup.indexOf("_") > -1) {
                        if ($.inArray(sGroup, properties.asExpandedGroups2) == -1 && !sGroup.endsWith("-na"))
                            return true;
                        else
                            return false;
                    }
            ...
            function fnExpandGroup(sGroup) {
                ...
                if (properties.oHideEffect != null)
                    ...
                    //martin ng -- close sub group by default.
                    if (sGroup.indexOf("_") == -1 && properties.bExpandableGrouping2) {
                        $(".group-item-" + sGroup, oTable).each(function () {
                            var oSubGroup = $(this).find("td.subgroup").first();
                            if (oSubGroup != null) {
                                var subGroup = oSubGroup.attr("data-group");
                                if (subGroup !== undefined) {
                                    if (subGroup.endsWith("-na")) {
                                        fnExpandGroup(subGroup);
                                        $(this).hide();
                                    }
                                    else {
                                        fnCollapseGroup(subGroup);
                                        $(this).show();
                                    }
                                }
                            }
            ...
            function _fnOnGroupClick(e) {
                ...
                if (properties.bExpandSingleGroup) {
                    ...
                    if (bIsExpanded) {
                        ...
                        properties.fnAfterGroupClicked(sGroup, iGroupLevel, "Close");
                    } else {
                        fnExpandGroup(sGroup);
                        properties.fnAfterGroupClicked(sGroup, iGroupLevel, "Open");
                    }
                }
                e.preventDefault();
                ...
                if (oSettings.aiDisplay.length > 0) {
                    for (var i = 0; i < nTrs.length; i++) {
                        ...
                        if (bUseSecondaryGrouping) {
                            ...
                            //martin ng
                            if (_fnIsGroupCollapsed(oGroup2.dataGroup) && !properties.bUseFilteringForGrouping) {
                                $(nTrs[i]).hide();
                            }
