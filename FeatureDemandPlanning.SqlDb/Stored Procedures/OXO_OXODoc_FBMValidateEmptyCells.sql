CREATE PROCEDURE [OXO_OXODoc_FBMValidateEmptyCells]
  @p_doc_id int,
  @p_prog_id int,
  @p_mode nvarchar(2),
  @p_object_id int,
...
BEGIN
	...
	BEGIN
		...
		(
			...
			FROM dbo.FN_OXO_Data_Get_FBM_Market(@p_doc_id, null, @p_object_id, @_model_id)
		...
		SELECT @p_doc_id, @p_prog_id, @p_mode, @p_object_id, @p_Rule_Id, C.ModelId, 
			  'Feature Main Body Section: ' + CAST(diff AS NVARCHAR(6)), 0, 'system', GETDATE()
		FROM SET_C C
		WHERE Diff > 0;
	...
	BEGIN
		...
		(
			SELECT COUNT(*) AS RecCount, Model_Id AS ModelId
			FROM dbo.FN_OXO_Data_Get_FBM_MarketGroup(@p_doc_id, @p_object_id, @_model_id)
	...
	BEGIN
		...
		(
			SELECT COUNT(*) AS RecCount, Model_Id AS ModelId
			FROM dbo.FN_OXO_Data_Get_FBM_Global(@p_doc_id, @_model_id)
			WHERE OXO_Code IS NOT NULL
			GROUP BY Model_Id
		),
		SET_C AS
...
BEGIN
	...
	BEGIN
		...
		(
			...
			INNER JOIN dbo.FN_SPLIT_MODEL_IDS(@_model_Id) MI
			ON PM.Id = MI.Model_Id
			CROSS JOIN OXO_Archived_Programme_Feature_Link FL
			WHERE PM.Programme_Id = @p_prog_id
			AND PM.Doc_Id = @p_doc_id
			AND PM.Active = 1
			AND FL.Programme_Id = @p_prog_id
			AND FL.Doc_Id = @p_doc_id
			GROUP BY  PM.ID
		),
		SET_B AS
		(
