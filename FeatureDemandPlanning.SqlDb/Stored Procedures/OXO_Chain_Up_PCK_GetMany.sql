CREATE PROCEDURE [OXO_Chain_Up_PCK_GetMany]
@p_doc_id int,
@p_prog_id int,
@p_model_id int,
@p_pack_id int,
...
AS
  ...
  DECLARE @tempTab TABLE(Level nvarchar(20), Level_Name nvarchar(500), Level_Id int, 
                         Model_Id int, Pack_Id int, OXO_Code nvarchar(50));
  DECLARE @_generic_value NVARCHAR(10);
  DECLARE @_group_value NVARCHAR(10);
  DECLARE @_market_value NVARCHAR(10);
  
  -- Do generic 
    INSERT INTO @tempTab VALUES ('Generic', 'Global Generic', -1, @p_model_id, @p_pack_id, null);
    SELECT @_generic_value = ISNULL(D.OXO_Code, '') 	
    FROM  @tempTab T
    LEFT JOIN OXO_Item_Data_PCK D WITH(NOLOCK)
  ...
  BEGIN
	INSERT INTO @tempTab 
		SELECT Top 1 'Group', Market_Group_Name, @p_object_id, @p_model_id, @p_pack_id, null
	...
	SELECT @_group_value = COALESCE(DG.OXO_Code, @_generic_value + '*', '') 	
		FROM  @tempTab T
		LEFT JOIN OXO_Item_Data_PCK DG WITH(NOLOCK)
  ...
  BEGIN
     INSERT INTO @tempTab 
		 SELECT Top 1 'Group', Market_Group_Name, Market_Group_Id, @p_model_id, @p_pack_id, null
		 FROM dbo.FN_Programme_Markets_Get(@p_prog_id, @p_doc_id)
		 WHERE Programme_Id = @p_prog_id
	...
	INSERT INTO @tempTab 
		 SELECT Top 1 'Market', Market_Name, @p_object_id, @p_model_id, @p_pack_id, null
		 FROM dbo.FN_Programme_Markets_Get(@p_prog_id, @p_doc_id)
		 WHERE Programme_Id = @p_prog_id
		 AND Market_Id = @p_object_id;	 	
	...
	SELECT @_market_value = COALESCE(DM.OXO_Code, @_group_value + '**', @_generic_value + '*', '') 	
		FROM  @tempTab T
		LEFT JOIN OXO_Item_Data_PCK DM
  ...
  SELECT T.Level, T.Level_Name AS LevelName, T.Model_Id as ModelId, T.Pack_Id AS PackId,
         T.OXO_Code AS OXOCode
  FROM @tempTab T