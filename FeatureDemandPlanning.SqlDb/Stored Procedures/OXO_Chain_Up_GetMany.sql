CREATE PROCEDURE [dbo].[OXO_Chain_Up_GetMany]
@p_doc_id int,
@p_prog_id int,
@p_model_id int,
@p_feature_id int,
...
AS
  ...
  BEGIN
	...
	SELECT @_group_value = COALESCE(DG.OXO_Code, @_generic_value + '*', '') 	
		FROM  @tempTab T
		LEFT JOIN OXO_Item_Data_VW DG WITH(NOLOCK)
  ...
  SELECT T.Level, T.Level_Name AS LevelName, T.Model_Id as ModelId, M.Name AS ModelName,
         T.Feature_Id as FeatureId, F.Description AS FeatureName, T.OXO_Code AS OXOCode
  FROM @tempTab T
  INNER JOIN OXO_Feature_Ext F
  ON T.Feature_Id = F.ID
  INNER JOIN dbo.FN_Programme_Models_Get(@p_prog_id, @p_doc_id) M
  ON T.Model_Id = M.Id
