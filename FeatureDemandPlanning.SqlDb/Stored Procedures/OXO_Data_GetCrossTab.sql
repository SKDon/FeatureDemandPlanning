CREATE PROCEDURE [dbo].[OXO_Data_GetCrossTab] 
  @p_make nvarchar(50),
  @p_doc_id int,
  @p_prog_id int,
...
AS
  ...
  BEGIN
	  ...
	  SET @sql = @sql +	'0 AS Variant_Count, MGM.Market_Group_Id, MGM.SubRegion, MGM.SubRegionOrder ';
	  ...
	  SET @sql = @sql + 'WHERE MGM.Programme_Id = @progId) '
  ...
  BEGIN	
		...
		BEGIN
			...
			SET @sql = @sql + 'PCK.Rule_Text AS RuleText, ';
			SET @sql = @sql + 'PCK.Extra_Info AS Comment, ';
  ...
  BEGIN	
		...
		BEGIN
			...
			SET @sql = @sql + 'PCK.FeatureRuleText AS RuleText, ';						
			...
			SET @sql = @sql + 'WHERE PCK.ProgrammeId = @progId) '; 
			SET @sql = @sql + 'SELECT * FROM SET_A  ';
			SET @sql = @sql + 'PIVOT ( MAX(OXO_Code) FOR Model_Id IN (@cols)) AS DataSet ';
			SET @sql = @sql + 'ORDER BY PackId, PackName, FeatureCode ';
			SET @sql = REPLACE(@sql, '@docId', @p_doc_id); 
		...
		BEGIN
			...
			SET @sql = @sql + 'PCK.FeatureRuleText AS RuleText, ';									
			SET @sql = @sql + 'PCK.LongDescription, ';						
			SET @sql = @sql + 'ODPF.OXO_Code,';
			SET @sql = @sql + 'ODPF.Model_Id ';
			SET @sql = @sql + 'FROM OXO_Pack_Feature_VW PCK '; 
  ...
  BEGIN	
     ...
     BEGIN
		...
		SET @sql = @sql + 'LongDescription, DisplayOrder, EFGName FROM OXO_Programme_Feature_VW ';
		SET @sql = @sql + 'WHERE ProgrammeId = @progId ), '; 
		...
		SET @sql = @sql +	'FEA.FeatureComment, FEA.SystemDescription, FEA.FeatureRuleText AS RuleText, FEA.LongDescription, '; 
  ...
  BEGIN	
    ...
    SET @sql = @sql + 'SELECT Id, ProgrammeId, FeatureGroup, FeatureSubGroup, FeatureCode, '; 
	SET @sql = @sql + 'SystemDescription, BrandDescription, FeatureComment, FeatureRuleText, ';
	SET @sql = @sql + 'LongDescription, DisplayOrder, EFGName FROM OXO_Programme_GSF_VW ';
	SET @sql = @sql + 'WHERE ProgrammeId = @progId ), '; 
	SET @sql = @sql + 'SET_B AS (';	
	SET @sql = @sql + 'SELECT  -1000 AS ID, @progId AS ProgrammeId, Group_Name AS FeatureGroup,'; 
    SET @sql = @sql + 'null AS FeatureSubGroup,null  AS FeatureCode,';
    ...
    SET @sql = @sql + ' UNION SELECT * FROM SET_A ),';
	SET @sql = @sql + 'SET_C AS ( ';
	SET @sql = @sql +	'SELECT DISTINCT ISNULL(FEA.DisplayOrder, 100) AS Display_Order, '; 
	SET @sql = @sql +	'FEA.FeatureGroup,FEA.FeatureSubGroup,FEA.FeatureCode,FEA.BrandDescription,FEA.ID AS Feature_Id, ';	 
	SET @sql = @sql +	'FEA.FeatureComment, FEA.LongDescription, 0 AS RuleCount, FEA.EFGName, '; 
