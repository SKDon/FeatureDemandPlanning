...
CREATE VIEW [dbo].[Fdp_Import_VW] AS
	...
	SELECT 
		...
		, CAST( CASE 
					WHEN DMAP.ProgrammeId IS NULL AND SF.FdpSpecialFeatureId IS NULL
					THEN 1 
					ELSE 0
				END AS BIT)									AS IsDerivativeMissing
		, CAST( CASE 
					WHEN TMAP.ProgrammeId IS NULL AND SF.FdpSpecialFeatureId IS NULL
					THEN 1 
					ELSE 0
