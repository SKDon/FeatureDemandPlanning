...
CREATE VIEW [dbo].[OXO_Programme_Feature_VW]
AS
    ...
    INNER JOIN dbo.OXO_Programme				P ON	V.ID			= P.Vehicle_Id
    INNER JOIN dbo.OXO_Programme_Feature_Link	L ON	P.Id			= L.Programme_Id
    INNER JOIN dbo.OXO_Feature_Ext				F ON	L.Feature_Id	= F.ID
    INNER JOIN OXO_Feature_Group				G ON	F.OXO_Grp		= G.ID
    LEFT JOIN dbo.OXO_Feature_Brand_Desc		M ON	F.Feat_Code		= M.Feat_Code
												  AND	M.Brand			= V.Make
