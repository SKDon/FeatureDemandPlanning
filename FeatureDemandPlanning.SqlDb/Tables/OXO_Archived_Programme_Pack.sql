...
CREATE NONCLUSTERED INDEX [Idx_Archived_Prog_Pack]
    ON [dbo].[OXO_Archived_Programme_Pack]([Doc_Id] ASC, [Programme_Id] ASC);


GO
CREATE TRIGGER [dbo].[Tri_Archived_Programme_Pack]
ON [dbo].[OXO_Archived_Programme_Pack]
FOR INSERT, DELETE 
...
AS BEGIN
   ...
   IF EXISTS (SELECT * FROM INSERTED) AND NOT EXISTS(SELECT * FROM DELETED)
	BEGIN
		...
		SELECT 'Add Archived Programme Pack', Doc_Id, 'Document', 
		       Id, 'Pack',	  
   ...
   IF EXISTS(SELECT * FROM DELETED) AND NOT EXISTS (SELECT * FROM INSERTED)
	BEGIN 
		INSERT INTO OXO_Event_Log(Event_Type, Event_Parent_Id, Event_Parent_Object, 
		                          ...
		                          Event_Parent3_Id, Event_Parent3_Object,
		                          Event_CDSID, Event_DateTime, Changeset_Id) 
		SELECT 'Remove Archived Programme Pack', Doc_Id, 'Document', 
		       Id, 'Pack',	
