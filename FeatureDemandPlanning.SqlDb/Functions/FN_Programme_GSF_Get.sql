CREATE FUNCTION [dbo].[FN_Programme_GSF_Get]( 
	@p_prog_id INT,
    @p_doc_id INT
) 
...
BEGIN
  ...
  BEGIN
  	INSERT INTO @result
    SELECT DISTINCT Id, ProgrammeId, Make, FeatureGroup, FeatureSubGroup, FeatureCode, BrandDescription, 
                    FeatureComment, FeatureRuleText , LongDescription, DisplayOrder
	FROM OXO_Programme_GSF_VW
	WHERE ProgrammeId = @p_prog_id;
	
	INSERT INTO @result
	SELECT  -1000, @p_prog_id, @p_make, Group_Name, NULL AS Sub_Group_Name, '','No Feature Selected', null, null, null, Max(Display_Order) AS Display_Order
	FROM OXO_Feature_Group G
	WHERE Status = 1
	AND NOT EXISTS 
	(
		SELECT 1 
		FROM OXO_Programme_GSF_VW F
  ...
  ELSE
	...
	SELECT DISTINCT Id, ProgrammeId, Make, FeatureGroup, FeatureSubGroup, FeatureCode, BrandDescription, 
	                FeatureComment, FeatureRuleText, LongDescription, DisplayOrder
	FROM dbo.OXO_Archived_Programme_GSF_VW
	WHERE ProgrammeId = @p_prog_id
	AND doc_id = @p_doc_id;
