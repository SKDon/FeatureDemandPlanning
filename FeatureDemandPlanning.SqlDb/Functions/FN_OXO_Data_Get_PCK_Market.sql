CREATE FUNCTION [FN_OXO_Data_Get_PCK_Market] 
(
  @p_doc_id INT,
...
(
	...
	(
		...
		UNION
		SELECT Pack_Id, Model_Id FROM Market
	)
	SELECT 0 as Feature_Id, C.Pack_Id, C.Model_Id, coalesce(MK.OXO_Code, M.OXO_Code + '**', G.OXO_Code + '*') AS OXO_Code
	FROM Combine C
