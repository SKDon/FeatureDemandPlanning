...
namespace FeatureDemandPlanning.Schematron
{
	...
	public class SchemaLoader
	{
		...
		private void LoadAsserts(Rule rule, XPathNavigator context)
		{
			...
			{
                string test = asserts.Current.GetAttribute("test", String.Empty);
                string id = asserts.Current.GetAttribute("id", String.Empty);
                string area = asserts.Current.GetAttribute("area", String.Empty);
                string src = asserts.Current.GetAttribute("source", String.Empty);     

				if (asserts.Current is IHasXmlNode)
				{
                    string message = ((IHasXmlNode)asserts.Current).GetNode().InnerXml;
					Assert asr = rule.CreateAssert(test, src, id, area, message);
					asr.SetContext(_schema.NsManager);
					rule.Asserts.Add(asr);
				}
				else
				{
                    string message = asserts.Current.Value;
                    Assert asr = rule.CreateAssert(test, src, id, area, message);
					asr.SetContext(_schema.NsManager);
					rule.Asserts.Add(asr);
				}
		...
		private void LoadReports(Rule rule, XPathNavigator context)
		{
			...
			{
                string test = reports.Current.GetAttribute("test", String.Empty);
                string id = reports.Current.GetAttribute("id", String.Empty);
                string area = reports.Current.GetAttribute("area", String.Empty);
                string src = reports.Current.GetAttribute("source", String.Empty);   
    			
				if (reports.Current is IHasXmlNode)
				{
                    string message = ((IHasXmlNode)reports.Current).GetNode().InnerXml;
                    Report rpt = rule.CreateReport(test, src, id, area, message);
					rpt.SetContext(_schema.NsManager);
					rule.Reports.Add(rpt);
				}
				else
				{
                    string message = reports.Current.Value;
                    Report rpt = rule.CreateReport(test, src, id, area, message);
					rpt.SetContext(_schema.NsManager);
					rule.Reports.Add(rpt);
				}
